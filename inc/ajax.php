...
if($_POST['action'] == 'envoi_message') {
    $_POST['message'] = addslashes($_POST['message']); // évite les problèmes d'apostrophes.
     if(!empty($_POST['message'])) { // s'il y a bien un message, on l'enregistre
        $pdo->exec("INSERT INTO dialogue (id_membre, message, date) VALUES ('" . $_SESSION['membre']['id_membre'] . "', '$_POST[message]', NOW())"); // on insère le message dans la table dialogue
...
if($_POST['action'] == 'affichage_message') {
    $lastid = floor($lastid);
    $resultat = $pdo->query("SELECT d.id_dialogue, m.pseudo, m.civilite, d.message, date_format(d.date, '%d/%m/%Y') as datefr, date_format(d.date, '%H:%i:%s') as heurefr FROM dialogue d, membre m WHERE d.id_dialogue > $lastid AND d.id_membre = m.id_membre ORDER BY d.date ASC"); 
    ...
    while ($donnees = $resultat->fetch(PDO::FETCH_ASSOC)){
		...
		$tab['lastid'] = $donnees['id_dialogue']; // au dernier tour de boucle, le dernier id_dialogue sera conservé en mémoire afin de ne pas réafficher des messages déjà sortis puisque cette portion de code est exécutée toutes les 5 secondes.
    }
    $tab['validation'] = 'ok';
}
