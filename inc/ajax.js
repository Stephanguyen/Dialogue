$(document).ready(function() { // function est appelée automatiquement en jQuery lorsque le DOM est chargé.
    ...
    // permet de mettre l'ascenseur en bas.
    var url = 'inc/ajax.php'; // url avec laquelle on va échanger dynamiquement
    var lastid = 0; // dernier id de dialogue
    var timer = setInterval(affichage_message, 13000); // interval de vérification pour lancer la fonctoin affichant les messages.
    ...
    function affichage_membre_connecte() { // cette fonction permet d'afficher les membres connectés au tchat
        $.post(url, { action: 'affichage_membre_connecte' }, function(data) { // $.post() : permet d'accéder au contenu du post, on passe l'action affichage_membre_connecte au fichier ajax.php (contenu dans la variable url)
        ...
        return false; // empêche que le formulaire soit soumis
    }
    ...
    $("#formulaire_tchat form").submit(function() { // lorsque je clique sur submit
        ...
        var message = $('#formulaire_tchat form textarea[name=message]').val(); // .val() nous permet de récupérer la valeur du textearea
        $.post(url, { action: 'envoi_message', message: message }, function(data) {
            if (data.validation == 'ok') {
                affichage_message(); // quand on ajoute unmessage, on en profite pour aller voir les derniers messages à afficher
                $('#formulaire_tchat form textarea[name=message]').val(''); // on vide le textarea poru éviter qu'un membre envoi deux fois le même message.
            } else {
                alert('Problème lors de l\'envoi de message');
            }
            timer = setInterval(affichage_message, 5000); // on remet le timer à 5000 ms
            hideLoader(); // on retire le loader une fois que l'envoi a été fait
        }, 'json');
        return false; // empêche que le formulaire soit soumis de la manière traditionnelle en PHP et que cela recharge toute la page.
    });
    // ==================================================================================================
    // FONCTION PERMETTANT DE CONVERTIR UN CODE SMILEY EN IMAGE
    function convertir_smiley() {
        $('#message_tchat p').each(function() { // chaque p va être analysé (grâce au each()).
    ...
    function affichage_message() { // cette fonction permet d'afficher les messages
        $.post(url, { action: 'affichage_message', lastid: lastid },
            function(data) { // $.post() permet d'accéder au contenu du post, on passe l'action affichage_message au fichier ajax.php (contenu dans la variable url) ainsi que le dernier id.
                if (data.validation == 'ok') {
                    $("#message_tchat").append(data.resultat); // on append le résultat dans la div qui à l'id "message_tchat"
                    lastid = data.lastid; // on récupère également le dernier id. data vient du fichier ajax.php car c'est le nom de la variable qu'on a donné à la variable dans la boucle permettant de sortir les messages. 
                ...
                hiderLoader(); // on retire le loader une fois que l'affichage a été fait
            }, 'json');
        return false; // empêche que le formulaire soit soumis
    }
    // ==================================================================================================
